...
import { t as i18nT, load as loadI18n } from "./i18n";
...
function injectLinkPreviews(root: HTMLElement): void {
	...
	for (const post of Array.from(posts)) {
		...
		if (!url) {
			...
			if (bareMatch && !bareMatch[0].startsWith(`${origin}/`))
				url = bareMatch[0];
		}
		if (!url) continue;
...
function insertPreviewCard(
	...
	if (ogp.title)
		html += `<div class="link-preview-title">${escapeHTML(ogp.title)}</div>`;
	if (ogp.description)
		html += `<div class="link-preview-desc">${escapeHTML(ogp.description)}</div>`;
	if (ogp.site_name)
		html += `<div class="link-preview-site">${escapeHTML(ogp.site_name)}</div>`;
	html += `</div>`;
