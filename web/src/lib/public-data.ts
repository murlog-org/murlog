...
export function buildPersonaContext(
	...
	return {
		displayName:
			(persona.display_name as string) || (persona.username as string) || "",
...
export function buildPersonaTemplateData(
	persona: R,
	ctx: PersonaContext,
	domain: string,
	posts: R[],
...
export function mapPostsForTemplate(posts: R[], ctx: PersonaContext): R[] {
	return posts.map((p) => {
		...
		return {
			...
			avatarURL: useActorInfo
				? up.actor_avatar_url || ctx.avatarURL
				: ctx.avatarURL,
			displayName: useActorInfo
				? (up.actor_name as string) ||
					(up.actor_acct as string) ||
					ctx.displayName
				: ctx.displayName,
			handle: useActorInfo
				? (up.actor_acct as string) || ctx.handle
				: ctx.handle,
			profileURL: useActorInfo
				? up.actor_uri || ctx.profileURL
				: ctx.profileURL,
		};
	});
}
