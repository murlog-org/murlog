...
import { isLoggedIn as isGlobalLoggedIn, setLoggedIn as setGlobalLoggedIn } from "./lib/auth";
...
function isPublicPage(path: string): boolean {
  return path === "/" || path.startsWith("/users/") || path.startsWith("/tags/");
}
...
function HeaderMenu({ onAbout }: { onAbout: () => void }) {
  ...
  return (
    <DropdownMenu iconSize={18}>
      ...
      <a href="#" onClick={(e) => { e.preventDefault(); handleLogout(); }}>{t("my.logout") || "Logout"}</a>
      <a href="#" onClick={(e) => { e.preventDefault(); onAbout(); }}>{t("my.about") || "About"}</a>
    </DropdownMenu>
  );
}
...
function ProfileRouter({ username, path }: { username?: string; path?: string }) {
...
function FollowListRouter({ username, type }: { username?: string; path?: string; type: "following" | "followers" }) {
  if (username && username.startsWith("@") && username.includes("@", 1)) {
    if (isGlobalLoggedIn()) {
...
export function App() {
  ...
  return (
    <>
      {!hideHeader.has(path) && (
        <header class="header">
          <a href="/" class="header-brand" onClick={(e) => { e.preventDefault(); navigate("/"); }}>
            <Logo size={20} />
            <span>murlog</span>
          </a>
          {showAppHeader ? <HeaderMenu onAbout={() => setShowAbout(true)} /> : <PublicMenu onAbout={() => setShowAbout(true)} />}
        </header>
      )}
