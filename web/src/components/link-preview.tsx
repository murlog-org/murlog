...
async function flush(): Promise<void> {
  ...
  const requests = urls.map((url) => ({ method: "links.preview", params: { url } }));
...
export function LinkPreview({ content }: { content: string }) {
  ...
  return (
    <a href={ogp.url} class="link-preview" target="_blank" rel="noopener noreferrer">
      {ogp.image && <img class="link-preview-image" src={ogp.image} alt="" loading="lazy" />}
      <div class="link-preview-body">
        {ogp.title && <div class="link-preview-title">{ogp.title}</div>}
        {ogp.description && <div class="link-preview-desc">{ogp.description}</div>}
