...
import { mapPostsForTemplate, type PersonaContext } from "../lib/public-data";
import { activatePublic, type loadTheme, renderTemplate } from "../lib/theme";
...
export function usePublicTimeline(config: TimelineConfig) {
	...
	const renderPage = useCallback(
		(theme: ThemeCache, templateData: R) => {
			const html = renderTemplate(theme, config.templateName, templateData);
			if (ref.current && html) {
				ref.current.innerHTML = html;
				activatePublic();
				postsRef.current = ref.current.querySelector(
					".posts",
				) as HTMLDivElement;
			}
		},
