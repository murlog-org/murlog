...
export function FollowPage({ path }: { path?: string }) {
	...
	const handleLookup = async (e: Event) => {
		...
		setLookupError("");
		const { result, error } = await call<Account>("actors.lookup", {
			acct: acct.trim(),
		});
	...
	const isFollowing = (uri: string) =>
		follows.some((f) => f.target_uri === uri);
	...
	return (
		<div class="screen">
			...
			<div class="card">
				...
				{lookupError && (
					<p class="meta" style={{ color: "var(--danger)", marginTop: 8 }}>
						{lookupError}
					</p>
				)}
				...
				{lookupResult && (
					<div class="lookup-result">
						...
						<div class="lookup-info">
							<strong>
								{lookupResult.display_name || lookupResult.username}
							</strong>
							...
							{lookupResult.summary && (
								<div
									...
									dangerouslySetInnerHTML={{
										__html: sanitize(lookupResult.summary),
									}}
								/>
							)}
						</div>
						{isFollowing(lookupResult.uri || "") ? (
							...
							<button
								type="button"
								class="btn btn-primary btn-sm"
								onClick={() => handleFollow(lookupResult.uri || "")}
							>
								{t("my.follow.follow")}
							</button>
						)}
			...
			<div class="tabs">
				<a
					href="#"
					class={`tab${tab === "following" ? " active" : ""}`}
					onClick={(e) => {
						e.preventDefault();
						setTab("following");
					}}
				...
				<a
					href="#"
					class={`tab${tab === "followers" ? " active" : ""}`}
					onClick={(e) => {
						e.preventDefault();
						setTab("followers");
					}}
							...
							<div class="user-card" key={f.id}>
								<div class="user-card-main">
									...
									<div class="user-card-info">
										<a
											href={`/users/${displayAcct(f.acct, f.target_uri)}`}
											class="post-author-name"
										>
											{f.display_name || displayName(f.acct, f.target_uri)}
										</a>
										<span class="handle">
											{displayAcct(f.acct, f.target_uri)}
										</span>
										{f.summary && (
											<div
												class="meta"
								...
								<div class="user-card-actions">
									<span
										class={`badge ${f.accepted ? "badge-done" : "badge-pending"}`}
									>
										{f.accepted
											? t("my.follow.accepted")
											: t("my.follow.pending")}
									</span>
									<button
										type="button"
							...
							return (
								<div class="user-card" key={f.id}>
									<div class="user-card-main">
										...
										<div class="user-card-info">
											<a
												href={`/users/${displayAcct(f.acct, f.actor_uri)}`}
												class="post-author-name"
											>
												{f.display_name || displayName(f.acct, f.actor_uri)}
											</a>
											<span class="handle">
												{displayAcct(f.acct, f.actor_uri)}
											</span>
											{f.summary && (
												<div
													class="meta"
									...
									>
										...
										{isPending ? (
											...
											<button
												type="button"
												class="btn btn-outline btn-sm"
												onClick={() => handleFollow(f.actor_uri)}
											>
												{t("my.follow.follow")}
											</button>
										)}
