// Public follow/follower list page — displays follows or followers with infinite scroll.
// 公開フォロー/フォロワー一覧ページ — 無限スクロール対応。
...
export function PublicFollowList({ username, type = "following" }: Props) {
    ...
    <div class="screen">
      <div class="card">
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
          <a href={`/users/${username}`} style={{ opacity: 0.6 }}>&larr;</a>
        ...
        {items.length === 0 ? (
          ...
          items.map((item) => {
            // 認証済み → murlog 上のリモートプロフィール、未認証 → オリジナル URL。
            // Logged in → local remote profile, not logged in → original URL.
            const localHref = `/users/${displayAcct(item.acct, item.uri)}`;
            const href = isLoggedIn() ? localHref : (item.uri.startsWith("http") ? item.uri : localHref);
            const external = !isLoggedIn() && item.uri.startsWith("http");
            return (
            <div class="user-card" key={item.uri}>
              <div class="user-card-main">
                ...
                <div class="user-card-info">
                  <a href={href} {...(external ? { target: "_blank", rel: "noopener" } : {})} class="post-author-name">{displayName(item)}</a>
