...
export function PublicFollowList({ username, type = "following" }: Props) {
	...
	useEffect(() => {
		...
		(async () => {
			if (isRemote) {
				...
				const method =
					type === "following" ? "actors.following" : "actors.followers";
	...
	const loadMore = useCallback(async () => {
		...
		const method =
			type === "following" ? "actors.following" : "actors.followers";
		const res = await call<RemoteCollectionResult>(method, {
			acct,
	...
	return (
		<div class="screen">
			<div class="card">
				...
				>
					<a href={`/users/${username}`} style={{ opacity: 0.6 }}>
						&larr;
					</a>
					<h2 style={{ margin: 0 }}>
						{title}
							...
							<div class="user-card" key={item.uri}>
								<div class="user-card-main">
									...
									<div class="user-card-info">
										...
										<span class="handle">
											{displayAcct(item.acct, item.uri)}
										</span>
										{item.summary && (
											<div
												class="meta"
