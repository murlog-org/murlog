...
export function DomainFailuresPage({ path }: { path?: string }) {
  ...
  const load = useCallback(async () => {
    await loadI18n();
    const { result, error: err } = await call<DomainFailure[]>("domains.list_failures");
  ...
  return (
    <div class="screen screen-wide">
      <h2 class="page-title">{t("my.domain_failures.title") || "Domain Failures"}</h2>
          ...
          <table class="queue-table" style={{ tableLayout: "fixed" }}>
            ...
            <thead>
              <tr>
                ...
                <th class="queue-desktop">{t("my.domain_failures.error") || "Last error"}</th>
                <th class="queue-desktop">{t("my.domain_failures.last_at") || "Last failure"}</th>
                <th></th>
              </tr>
            </thead>
            <tbody>
              {failures.map((f) => (
                <tr key={f.domain} class={f.dead ? "row-failed" : ""}>
                  <td style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}><code>{f.domain}</code></td>
                  <td>{f.failure_count}</td>
                  <td>
                    <span class={`badge ${f.dead ? "badge-failed" : "badge-pending"}`}>
                      {f.dead ? "dead" : "degraded"}
                    </span>
                  </td>
                  <td class="queue-desktop error-cell">{f.last_error}</td>
                  <td class="queue-desktop meta">{formatTime(f.last_failure_at)}</td>
                  <td>
                    <button class="btn btn-outline btn-sm" onClick={() => handleReset(f.domain)}>
