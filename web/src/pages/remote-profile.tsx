...
export function RemoteProfile({ acct }: Props) {
	...
	const loadMore = useCallback(async () => {
		...
		setLoading(true);
		const res = await call<OutboxResult>("actors.outbox", {
			acct,
	...
	useEffect(() => {
		...
		(async () => {
			// 1. Get actor data.
			const ssrData = getSSRData<SSRRemoteData>();
			...
			(async () => {
				const checkRes = await call<Follow | null>("follows.check", {
					target_uri: resolvedActor!.uri,
				});
				...
				const renderButton = (isFollowing: boolean, followId?: string) => {
					...
					btn.addEventListener("click", async () => {
						...
						if (isFollowing && followId) {
							...
							const res = await call<Follow>("follows.create", {
								target_uri: resolvedActor!.uri,
							});
			...
			if (resolvedActor.featured_url) {
				batchRequests.push({
					method: "actors.featured",
					params: { featured_url: resolvedActor.featured_url },
				});
			}
			const batchResults = await callBatch(batchRequests);
			const outboxRes = batchResults[0] as {
				result?: OutboxResult;
				error?: unknown;
			};
			const featuredRes = batchResults[1] as
				| { result?: OutboxPost[] }
				| undefined;
			...
			const merged = [
				...pinnedPosts,
				...outboxPosts.filter((p) => !featuredURIs.has(p.uri)),
			];
			if (merged.length > 0) {
				appendPosts(merged);
			} else {
