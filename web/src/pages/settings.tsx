    ...
    <div class="screen">
      ...
      <div class="card card-section">
        <form onSubmit={handleSave}>
          <h3 class="card-section-title">{t("my.settings.profile") || "Profile"}</h3>
          ...
          <div class="input-group" style={{ marginTop: 16 }}>
            <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer" }}>
              <input
                ...
                style={{ width: "auto" }}
              />
              {t("my.settings.locked") || "Lock account (manually approve followers)"}
            </label>
            <p class="meta" style={{ marginTop: 4 }}>
              {t("my.settings.locked_desc") || "When enabled, you must approve each follow request."}
            </p>
          </div>
          ...
          <div class="input-group" style={{ marginTop: 16 }}>
            <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer" }}>
              <input
                type="checkbox"
                checked={showFollows}
                onChange={(e) => setShowFollows((e.target as HTMLInputElement).checked)}
                style={{ width: "auto" }}
              />
              {t("my.settings.show_follows") || "Show follow/follower lists on profile"}
            </label>
            <p class="meta" style={{ marginTop: 4 }}>
              {t("my.settings.show_follows_desc") || "When disabled, only counts are visible. Lists are hidden from the public profile and ActivityPub."}
            </p>
          </div>
          ...
          <div class="input-group" style={{ marginTop: 16 }}>
            <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer" }}>
              <input
                ...
                onChange={(e) => setDiscoverable((e.target as HTMLInputElement).checked)}
                style={{ width: "auto" }}
              />
              {t("my.settings.discoverable") || "Appear in search results"}
            </label>
            <p class="meta" style={{ marginTop: 4 }}>
              {t("my.settings.discoverable_desc") || "Allow other servers to discover your profile through search and directory listings."}
            </p>
          </div>
      ...
      <div class="card card-section">
        <h3 class="card-section-title">{t("my.settings.password") || "Change Password"}</h3>
        <form
          onSubmit={async (e) => {
            e.preventDefault();
            setPwError("");
            ...
            if (newPassword !== confirmPassword) {
              setPwError(
                t("my.settings.password_mismatch") ||
...
function TOTPSettings() {
  ...
  const handleSetup = async () => {
    setError("");
    setSuccess("");
    const { result, error: err } = await call<{ secret: string; uri: string }>("totp.setup");
  ...
  return (
    <div class="card card-section">
      <h3 class="card-section-title">{t("my.settings.totp") || "Two-Factor Authentication"}</h3>
      ...
      {!enabled && !setupData && (
        <button class="btn btn-outline" onClick={handleSetup}>
      ...
      {setupData && (
        <form onSubmit={handleVerify}>
          <p class="meta" style={{ marginBottom: 8 }}>
            {t("my.settings.totp_desc") || "Scan the QR code with your authenticator app."}
          </p>
          <canvas ref={canvasRef} style={{ display: "block", margin: "12px 0" }} />
          <div class="meta" style={{ fontSize: 11, wordBreak: "break-all", marginBottom: 8 }}>
            {setupData.secret}
          </div>
          <div class="input-group">
            <input
      ...
      {enabled && (
        <button class="btn btn-outline" onClick={handleDisable} style={{ marginTop: 8 }}>
          {t("my.settings.totp_disable") || "Disable"}
        </button>
      )}

      {error && <div class="error" style={{ marginTop: 8 }}>{error}</div>}
      {success && <div style={{ color: "var(--accent)", fontSize: 13, marginTop: 8 }}>{success}</div>}
    </div>
  );
}
...
function DBMaintenance() {
  ...
  const handleVacuum = async () => {
    ...
    setError("");
    setResult("");
    const { result: res, error: err } = await call<{ deleted: number }>("queue.vacuum");
    setVacuuming(false);
    if (err) {
      setError(err.message);
    } else if (res) {
      setResult(`${t("my.settings.vacuum_done") || "Cleanup complete"}: ${res.deleted} ${t("my.settings.vacuum_deleted") || "jobs deleted"}`);
    }
  };

  return (
    <div class="card card-section">
      <h3 class="card-section-title">{t("my.settings.backup") || "Database Backup"}</h3>
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        <a href="/api/mur/v1/backup" class="btn btn-outline" target="_blank" download>
          {t("my.settings.backup_download") || "Download"}
        </a>
        <button class="btn btn-outline" onClick={handleVacuum} disabled={vacuuming}>
          {vacuuming ? "..." : t("my.settings.vacuum") || "Cleanup & Vacuum"}
        </button>
      </div>
      {error && <div class="error" style={{ marginTop: 8 }}>{error}</div>}
      {result && <div style={{ color: "var(--accent)", fontSize: 13, marginTop: 8 }}>{result}</div>}
    </div>
  );
}
...
function CrawlerSettings() {
  ...
  useEffect(() => {
    call<{ robots_noindex: boolean; robots_noai: boolean }>("site.get_settings").then(({ result }) => {
      if (result) {
  ...
  const handleSave = async () => {
    setSaving(true);
    setSuccess("");
    await call("site.update_settings", { robots_noindex: noIndex, robots_noai: noAI });
    setSaving(false);
  ...
  return (
    <div class="card card-section">
      <h3 class="card-section-title">{t("my.settings.crawlers") || "Crawler Settings"}</h3>
      <div class="input-group">
        <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer" }}>
          <input
            type="checkbox"
        ...
        <p class="meta" style={{ marginTop: 4 }}>
          {t("my.settings.robots_noindex_desc") || "Prevent search engines like Google from indexing your site."}
        </p>
      </div>
      <div class="input-group" style={{ marginTop: 12 }}>
        ...
        <p class="meta" style={{ marginTop: 4 }}>
          {t("my.settings.robots_noai_desc") || "Block AI crawlers (GPTBot, CCBot, etc.) from using your content for training."}
        </p>
      </div>
      <div class="card-section-actions">
