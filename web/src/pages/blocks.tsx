...
export function BlocksPage({ path }: { path?: string }) {
	...
	const handleBlockActor = async (e: Event) => {
		...
		setError("");
		const { error: err } = await call("blocks.create", {
			actor_uri: actorInput.trim(),
		});
		if (err) {
	...
	const handleBlockDomain = async (e: Event) => {
		...
		setError("");
		const { error: err } = await call("domain_blocks.create", {
						...
						<form onSubmit={handleBlockActor}>
							<div class="input-group">
								...
								<input
									...
									value={actorInput}
									onInput={(e) =>
										setActorInput((e.target as HTMLInputElement).value)
									}
						...
						<div class="card user-card" key={b.id}>
							...
							<div class="user-card-actions">
								<button
									...
									onClick={() => handleUnblockActor(b.actor_uri)}
								>
									{t("my.blocks.unblock")}
								</button>
							</div>
						...
						<form onSubmit={handleBlockDomain}>
							<div class="input-group">
								...
								<input
									...
									value={domainInput}
									onInput={(e) =>
										setDomainInput((e.target as HTMLInputElement).value)
									}
