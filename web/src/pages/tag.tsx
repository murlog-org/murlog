...
export function TagPage({ tag }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (posts.length === 0) {
        fetches.push(call<Record<string, unknown>[]>("posts.list_by_tag", { tag, limit: PAGE_SIZE }));
      }
      ...
      if (posts.length === 0 && results[2]) {
        posts = (results[2] as { result?: Record<string, unknown>[] }).result ?? [];
      }

      let ctx = buildPersonaContext({}, location.hostname, location.origin);
      if (personasRes.result && personasRes.result.length > 0) {
        const persona = personasRes.result.find((p) => p.primary) || personasRes.result[0];
