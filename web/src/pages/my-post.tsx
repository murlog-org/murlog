...
export function MyPost({ id: postId }: Props) {
  ...
  const loadPost = useCallback(() => {
    ...
    loader.run(async () => {
      await loadI18n();
      const t = await callOrThrow<ThreadData>("posts.get_thread", { id: postId });
      setThread(t);
    });
  }, [postId, loader.run]);
  ...
  const updatePost = (id: string, updater: (p: Post) => Post) => {
    setThread((prev) => {
      ...
      return {
        ...
        descendants: prev.descendants.map((p) => p.id === id ? updater(p) : p),
      };
    });
  };
  ...
  const handleFavourite = async (p: Post) => {
    ...
    if (error) return;
    updatePost(p.id, (prev) => ({ ...prev, favourited: !prev.favourited, favourites_count: prev.favourites_count + (prev.favourited ? -1 : 1) }));
  };
  ...
  const handleReblog = async (p: Post) => {
    ...
    const { error } = await call(method, { post_id: p.id });
    if (error) return;
    updatePost(p.id, (prev) => ({ ...prev, reblogged: !prev.reblogged, reblogs_count: prev.reblogs_count + (prev.reblogged ? -1 : 1) }));
  };
  ...
  const actions = { onFavourite: handleFavourite, onReblog: handleReblog, onReply: handleReply };
    ...
    <div class="screen">
      ...
      {replyTo && (
        <ComposeBox
          ...
          onPosted={() => { setReplyTo(null); loadPost(); }}
        />
      )}
        ...
        <>
          ...
          {thread.ancestors.map((p) => (
            <a key={p.id} href={`/my/posts/${p.id}`} class="thread-link">
              <PostCard post={p} actions={actions} expanded={expandedPosts.has(p.id)} onToggleExpand={toggleExpand} compact />
            </a>
          ))}

          {/* Main post / メイン投稿 */}
          <PostCard post={thread.post} actions={actions} expanded={expandedPosts.has(thread.post.id)} onToggleExpand={toggleExpand} />
