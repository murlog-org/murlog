...
describe("acctFromURI", () => {
  it("converts standard Mastodon URI", () => {
    expect(acctFromURI("https://mastodon.social/users/alice")).toBe("@alice@mastodon.social");
  });

  it("converts GoToSocial URI", () => {
    expect(acctFromURI("https://gts.example.com/users/bob")).toBe("@bob@gts.example.com");
  });

  it("converts URI with port", () => {
    expect(acctFromURI("https://localhost:3000/users/alice")).toBe("@alice@localhost:3000");
  });

  it("handles URI with nested path", () => {
    // Misskey-like: /users/internal-id — takes last segment.
    expect(acctFromURI("https://misskey.io/users/akptweh1lz3s02mg")).toBe("@akptweh1lz3s02mg@misskey.io");
  });
  ...
  it("handles Lemmy-style AP URI", () => {
    expect(acctFromURI("https://lemmy.world/u/username")).toBe("@username@lemmy.world");
  });
});
