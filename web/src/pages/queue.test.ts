...
describe("extractHost", () => {
	it("extracts host from actor_uri", () => {
		expect(
			extractHost(
				makeJob('{"actor_uri":"https://mastodon.social/users/alice"}'),
			),
	...
	it("extracts host from target_actor_uri", () => {
		expect(
			extractHost(
				makeJob('{"target_actor_uri":"https://gts.example.com/users/bob"}'),
			),
		).toBe("gts.example.com");
	});

	it("extracts host from target_uri", () => {
		expect(
			extractHost(makeJob('{"target_uri":"https://misskey.io/users/xyz"}')),
		).toBe("misskey.io");
	});
	...
	it("prefers actor_uri over target_actor_uri", () => {
		expect(
			extractHost(
				makeJob(
					'{"actor_uri":"https://a.com/u/x","target_actor_uri":"https://b.com/u/y"}',
				),
			),
		).toBe("a.com");
	});
	...
	it("handles URI with port", () => {
		expect(
			extractHost(makeJob('{"actor_uri":"https://localhost:3000/users/a"}')),
		).toBe("localhost");
	});
	...
	it("handles URI with subdomain", () => {
		expect(
			extractHost(
				makeJob('{"actor_uri":"https://ap.sub.example.co.uk/users/a"}'),
			),
		).toBe("ap.sub.example.co.uk");
	});
});
