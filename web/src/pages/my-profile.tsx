...
export function MyProfile({ username }: Props) {
	...
	const outboxToPost = useCallback(
		(p: OutboxPost, actorInfo: Account): Post => ({
			id: p.local_id || p.uri,
			persona_id: "",
			content: p.content,
			summary: p.summary,
		...
		loader.run(async () => {
			...
			if (isRemote && actorInfo) {
				const result = await callOrThrow<{
					posts: OutboxPost[];
					next?: string;
				}>("actors.outbox", { acct, limit: PAGE_SIZE });
				const fetched = (result.posts ?? []).map((p) =>
					outboxToPost(p, actorInfo!),
				);
				setPosts(fetched);
				nextCursorRef.current = result.next || null;
				setHasMore(!!result.next);
			} else if (!isRemote) {
				const fetched = await callOrThrow<Post[]>("posts.list", {
					username,
					limit: PAGE_SIZE,
					public_only: true,
				});
		...
		try {
			if (isRemote) {
				...
				const { result } = await call<{ posts: OutboxPost[]; next?: string }>(
					"actors.outbox",
					{ acct, cursor: nextCursorRef.current, limit: PAGE_SIZE },
				);
				...
				if (!lastId) return;
				const { result } = await call<Post[]>("posts.list", {
					username,
					cursor: lastId,
					limit: PAGE_SIZE,
					public_only: true,
	...
	const handleFollow = async () => {
		...
		try {
			if (following?.is && following.id) {
				...
				const res = await call<{ id: string }>("follows.create", {
					target_uri: actor.uri,
				});
				...
				<div class="card" style={{ padding: 0 }}>
					...
					<div class="profile-body">
						<div class="profile-identity">
							...
							<div class="profile-name">
								...
								<p class="handle">
									{isRemote
										? `@${acct}`
										: `@${actor.username}@${location.hostname}`}
								</p>
							</div>
							{isRemote && following && (
								<div
								...
								>
									...
									>
										{following.is
											? t("my.follow.unfollow") || "Unfollow"
											: t("my.follow.follow") || "Follow"}
									</button>
									{actor.uri && (
										<a
											href={actor.uri}
						...
						<div class="profile-stats">
							{!isRemote && (
								<span>
									<strong>{actor.post_count ?? 0}</strong> {t("public.posts")}
								</span>
							)}
							<a
								href={`/my/users/${username}/following`}
								class="profile-stat-link"
							>
								{!isRemote && <strong>{actor.following_count ?? 0}</strong>}{" "}
								{t("public.following")}
							</a>
							<a
								href={`/my/users/${username}/followers`}
								class="profile-stat-link"
							>
								{!isRemote && <strong>{actor.followers_count ?? 0}</strong>}{" "}
								{t("public.followers")}
							</a>
						</div>
					</div>
