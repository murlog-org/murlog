...
function ThreadPost({
	...
	return (
		<div class={`card ${highlight ? "thread-target" : "thread-post"}`}>
			{post.actor_uri && (
				<div class="meta" style={{ marginBottom: 4 }}>
					{post.actor_name || post.actor_uri}
				</div>
			)}
			<div
				class="post-content"
...
export function PublicPost({ username, id: postId }: Props) {
	...
	useEffect(() => {
		...
		(async () => {
			...
			const threadRes = await call<ThreadData>("posts.get_thread", {
				id: postId,
			});
			if (threadRes.result) {
			...
			if (!data) {
				...
				const postRes = await call<Record<string, unknown>>("posts.get", {
					id: postId,
				});
	...
	return (
		<div class="screen">
			...
			{thread?.ancestors && thread.ancestors.length > 0 && (
				<div class="thread-ancestors">
					{thread.ancestors.map((p) => (
						<a
							key={p.id}
							href={`/users/${username}/posts/${p.id}`}
							class="thread-link"
						>
							<ThreadPost post={p} />
						</a>
			...
			{/* Descendants / 子孫 */}
			{thread?.descendants && thread.descendants.length > 0 && (
